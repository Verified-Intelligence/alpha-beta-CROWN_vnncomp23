...
from utils import repeat, fast_hist_copy
...
class DomainUpdater:
    ...
    def get_num_copy(self, split, num_split, mode):
    ...
    def set_branched_bounds(self, d, split, mode='depth'):
        ...
        d['lower_bounds'] = {k: v.view(-1, *v.shape[2:])
                             for k, v in d['lower_bounds'].items()}
        d['upper_bounds'] = {k: v.view(-1, *v.shape[2:])
                             for k, v in d['upper_bounds'].items()}
