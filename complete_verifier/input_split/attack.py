import torch
import arguments
from attack.attack_pgd import pgd_attack_with_general_specs, test_conditions, default_adv_example_finalizer, save_cex, \
...
def attack_in_input_bab_parallel(model_ori, domains, x, vnnlib=None):
    ...
    cond_mat = [[Cs.shape[1] for _ in range(Cs.shape[0])]]
    ...
    attack_image, attack_output, _ = eval(arguments.Config["attack"]["adv_example_finalizer"])(model_ori, attack_x, best_deltas, ubs, lbs, Cs, rhs, cond_mat)
    ...
    if res.all() and arguments.Config["general"]["save_adv_example"]:
        try:
            save_cex(attack_image[:,idx].unsqueeze(1), attack_output[:,idx], attack_x.unsqueeze(1), vnnlib, arguments.Config["attack"]["cex_path"], ubs[:,idx].unsqueeze(1), lbs[:,idx].unsqueeze(1))
...
def massive_pgd_attack(x, model_ori, vnnlib=None):
    ...
    # print('num_restarts, num_steps', num_restarts, num_steps)
    best_deltas = pgd_attack_with_general_specs(
...
def check_adv(domains, model_ori, vnnlib=None):
    ...
    if res.all():
        ...
        if arguments.Config["general"]["save_adv_example"]:
            try:
                save_cex(attack_image[:, idx].unsqueeze(1), attack_output[:, idx], adv_example[:, idx].unsqueeze(1), vnnlib, arguments.Config["attack"]["cex_path"], data_max[:, idx].unsqueeze(1), data_min[:, idx].unsqueeze(1))
            except Exception as e:
                print(str(e))
                print('save adv example failed')
