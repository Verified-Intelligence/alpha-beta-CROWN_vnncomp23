...
default_optimize_bound_args = {
    ...
    # Output constraints require two flags: One to add the gammas to layers, and one to
    # actually activate them. We have to add gammas right at the beginning during the
    # construction of the network, but they'll only be used during the alpha optimization
    # stage.
    'include_gammas_in_layer': False,  # Needed if output constraints are ever to be used
    'include_output_constraint': False,  # Enable optimization w.r.t. output constraints.
...
def _get_optimized_bounds(
    ...
    include_output_constraint = opts['include_output_constraint']
    ...
    for i in range(iteration):
        ...
        if os.environ.get('AUTOLIRPA_DEBUG_OPT', False):
            print(f'****** iter [{i}]',
                  ...
                  f' pruning_in_iteration open status: {pruner and pruner.pruning_in_iteration}')
        ...
        if alpha:
            for m in optimizable_activations:
                m.clip_alpha()
            # Only need alpha_mask for the first iteration
...
def init_alpha(self: 'BoundedModule', x, share_alphas=False, method='backward',
    ...
    include_output_constraint = self.bound_opts['optimize_bound_args']['include_output_constraint']
    if (not skip_bound_compute or interm_bounds is None or
        ...
        with torch.no_grad():
            ...
            self.bound_opts['optimize_bound_args']['include_output_constraint'] = False
            ...
            self.bound_opts['optimize_bound_args']['include_output_constraint'] = (
                include_output_constraint
            )
    else:
    ...
    for node in optimizable_activations:
        ...
        if method in ['backward', 'forward+backward']:
            if include_output_constraint:
                # '/input' is correct for the YOLO benchmark
                # .input_name cannot be used, it provides a layer that's even closer to the input,
                # but self.get_alpha_crown_start_nodes cannot handle that one.
                backward_from_node = self['/input']
            else:
