...
def batched_backward(self: 'BoundedModule', node, C, unstable_idx, batch_size,
    ...
    :param bound_lower:
    :param bound_upper:
    :param return_A: overwrite the default return_A option. If it is None, inherit from self.return_A
    :return:
    """
...
def backward_general(
    ...
    update_mask=None,
    verbose=True,
    include_output_constraint: bool = False,
            ...
            if self.return_A and self.needed_A_dict and bound_node.name in self.needed_A_dict:
                ...
                if len(self.needed_A_dict[bound_node.name]) == 0 or l.name in self.needed_A_dict[bound_node.name]:
                    ...
                    A_record.update({l.name: {
                        "lA": (A[0][0] if isinstance(A[0][0], Patches) else A[0][0].transpose(0, 1).detach()) if A[0][0] is not None else None,
                        "uA": (A[0][1] if isinstance(A[0][1], Patches) else A[0][1].transpose(0, 1).detach()) if A[0][1] is not None else None,
    ...
    if self.return_A and self.needed_A_dict and bound_node.name in self.needed_A_dict:
        save_A_record(
            bound_node, A_record, self.A_dict, roots, self.needed_A_dict[bound_node.name],
        ...
        if not reduced_dim:
            ...
            if dim > crown_batch_size:
                ...
                newC = torch.eye(
                    ...
                    dtype=list(self.parameters())[0].dtype).unsqueeze(0).expand(
                        batch_size, -1, -1).view(
                            batch_size, dim, *node.output_shape[1:])
...
def get_alpha_crown_start_nodes(
    ...
    if backward_from_node != node:
        assert self.bound_opts['optimize_bound_args']['include_output_constraint']
