...
class BoundTwoPieceLinear(BoundOptimizableActivation):
    ...
    def bound_backward(self, last_lA, last_uA, x=None, start_node=None,
        ...
        if hasattr(x, "lower"):
            ...
            assert self.options["optimize_bound_args"]["include_output_constraint"]
    ...
    def _backward_relaxation(self, last_lA, last_uA, x, start_node, unstable_idx):
        ...
        if self.opt_stage in ['opt', 'reuse']:
            if getattr(self, 'alpha_mask_domains', False):
                ...
                alpha_init = self._relu_lower_bound_init(upper_d)
                self.alpha[start_node.name].data[:] = alpha_init

            # alpha_mask = getattr(self.inputs[0], 'alpha_mask', None)
            # if alpha_mask is not None and alpha_mask.nonzero().any():
            #     # Reinitialize masked alphas
            #     # CLEANUP For development, printing this message for now
            #     print('Reinitialize masked alphas', sqelf, alpha_mask.int().sum())
            #     assert unstable_idx is None
            #     alpha_init = self._relu_lower_bound_init(upper_d[alpha_mask])
            #     self.alpha[start_node.name].data[:, :, alpha_mask] = alpha_init
    ...
    def _backward_relaxation(self, last_lA, last_uA, x, start_node, unstable_idx):
        ...
        if self.opt_stage in ['opt', 'reuse']:
            if getattr(self, 'alpha_mask_domains', False):
                ...
                print('Reinitialize masked alphas', self)
                assert unstable_idx is None
                alpha_init = torch.zeros_like(upper, requires_grad=True)
                self.alpha[start_node.name].data[:] = alpha_init
