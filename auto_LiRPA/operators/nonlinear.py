...
from .base import epsilon, LinearBound, Patches, maybe_unfold_patches
...
class BoundOptimizableNonLinear(BoundTanh):
    ...
    def bound_relax_impl_sigmoid(self, lb, ub, func, dfunc):
        ...
        if self.opt_stage in ['opt', 'reuse']:
            ...
            if getattr(self, 'alpha_mask_domains', False):
                # Reinitialize masked alphas
                # CLEANUP For development, printing this message for now
                print('Reinitialize masked alphas', self)
                self.alpha[ns].data[:4, :] = (lower + upper)/2
                self.alpha[ns].data[4:6, :] = d_lower
                self.alpha[ns].data[6:8, :] = d_upper

            # alpha_mask = getattr(self.inputs[0], 'alpha_mask', None)
            # if alpha_mask is not None and alpha_mask.any():
            #     # Reinitialize masked alphas
            #     # CLEANUP For development, printing this message for now
            #     print('Reinitialize masked alphas', self, alpha_mask.int().sum())
            #     self.alpha[ns].data[:4, :, alpha_mask] = (
            #         lower[alpha_mask] + upper[alpha_mask])/2
            #     self.alpha[ns].data[4:6, :, alpha_mask] = d_lower[alpha_mask]
            #     self.alpha[ns].data[6:8, :, alpha_mask] = d_upper[alpha_mask]
            ...
            self.alpha[ns].data[4:6, :] = (
                self.convex_concave * torch.min(self.alpha[ns][4:6, :], d_lower)
                + self.concave_convex * torch.max(self.alpha[ns][4:6, :], d_lower))
            self.alpha[ns].data[6:8, :] = (
                self.convex_concave * torch.max(self.alpha[ns][6:8, :], d_upper)
                + self.concave_convex * torch.min(self.alpha[ns][6:8, :], d_upper))
    ...
    def bound_relax(self, x, init=False, dim_opt=None):
        ...
        if self.opt_stage in ['opt', 'reuse']:
            ...
            if getattr(self, 'alpha_mask_domains', False):
                ...
                # CLEANUP For development, printing this message for now
                print('Reinitialize masked alphas', self)
                self.alpha[self._start].data[:] = (x.lower + x.upper) / 2
            # if getattr(self, 'alpha_mask_domains', False):
            #     # Reinitialize masked alphas
            #     # CLEANUP For development, printing this message for now
            #     print('Reinitialize masked alphas', self)
            #     alpha_init = (x.lower + x.upper) / 2
            #     self.alpha[self._start].data[:] = alpha_init
